...
const getScores = async (validation) => {
  let response = '';
  if (validation) {
    response = await fetch('https://us-central1-js-capstone-backend.cloudfunctions.net/api/games/55wl7tfhhy2GwkyVLjmS/scores/', { mode: 'cors', method: 'GET' }).catch(errHandler);
  }
